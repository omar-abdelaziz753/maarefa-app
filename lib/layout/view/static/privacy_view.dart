  ...
  Widget build(BuildContext context) {
    return BlocProvider(
        create: (BuildContext context) =>
            StaticScreensCubit(StaticScreensRepository())..getPrivacy(),
        child: BlocConsumer<StaticScreensCubit, StaticScreensState>(
            listener: (context, state) {},
            builder: (context, state) {
              return BlocBuilder<StaticScreensCubit, StaticScreensState>(
                  builder: (context, state) {
                if (state is PrivacyLoadedState) {
                  final data = (state).data;
                  return privacyView(context: context, data: data);
            ...
            children: [
              Container(
                ...
                decoration: const BoxDecoration(
                  image: DecorationImage(image: AssetImage(privacy)),
                ),
              ),
              Space(
                boxHeight: 10.h,
